...
void main() {
  testWidgets(
    'shows folder scan summary and episode durations for video uploads',
    (tester) async {
      ...
      expect(
        find.text(
          'Found 2 supported video files ready to import and ignored 3 unsupported files.',
        ),
        findsOneWidget,
      );
  ...
  testWidgets(
    'shows clear empty-state copy when a folder contains no supported media',
    (tester) async {
      final state = AddMaterialState(
        type: study.MaterialType.video,
        title: '',
      ...
      expect(
        find.text(
          'Found no supported video files and ignored 4 unsupported files.',
        ),
        findsOneWidget,
      );
